...
// Updates the drive frequency and duty cycle for primary channel frequency control.
// Signal output on D5
...
//   TCC0->CC[1].reg = (uint8_t)((VARIANT_MCK / faimsfb.Freq) * (100 - faimsfb.Duty) / 100);
   TCC0->CC[1].reg = (uint16_t)((VARIANT_MCK / faimsfb.Freq) * (100 - faimsfb.Duty) / 100);
   //TCC0->WEXCTRL.bit.OTMX = 0;   // This works with default values but I am not sure why?
   while (TCC0->SYNCBUSY.bit.CC0 || TCC0->SYNCBUSY.bit.CC1);  
   // Set frequency, frequency = VARIANT_MCK / value
//   TCC0->PER.reg = (uint8_t)(VARIANT_MCK / faimsfb.Freq);
   TCC0->PER.reg = (uint16_t)(VARIANT_MCK / faimsfb.Freq);
...
// Sets the drive level for the output RF driver, 0 to 100%.
void UpdateDrive(float drive)
{
   // If drive is 0 then turn off the FET driving the flyback transformer
   if(drive == 0)
   {
      pinMode(5,OUTPUT);
      digitalWrite(5,HIGH);
   }
   else pinPeripheral(5, PIO_TIMER_ALT);
   if(drive < 0) drive = 0;
...
//  D5,   TC3/WO[1], TCC0/WO[5]   ,Frequency control 1
//  D3,   TCC0/WO[1], TCC1/WO[3]  ,Frequency control 2
//  PA13, TCC2/WO[1], TCC0/ WO[7] ,PWM drive level
//
void initPWM(void)
{
// Configure frequency control channel 1, TCC0/WO[5]
   pinPeripheral(5, PIO_TIMER_ALT);
   ...
   TCC0->PER.reg = 30; //0xFF;
   while (TCC0->SYNCBUSY.bit.PER);
   // Enable TCCx
   TCC0->CTRLA.bit.ENABLE = 1;
   while (TCC0->SYNCBUSY.bit.ENABLE);
   
// Configure frequency control channel 2
...
// Configure drive power PWM, set frequency to 50KHz
